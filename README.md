...
- `.github/workflows/deploy-pages.yml`: push / 수동 / 3시간마다 자동 배포
...
- 이후 Actions가 뉴스를 수집해 `news.json` 갱신
- `push`만 해도 Pages 재배포
- 수동 실행과 예약 실행 모두 가능
...
- 뉴스 자동 갱신 스케줄(3시간 간격) 추가
- NAVER API 키가 없거나 실패해도 RSS로 대체 수집
- 수집 실패 시 기존 `data/news.json` 유지
- 화면에서 `오늘의 뉴스` 대신 `최신 뉴스`와 실제 기준일 표시
